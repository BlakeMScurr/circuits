...
describe("credential old test", function () {
    this.timeout(200000);


    it("Test Credential simple tree", async () => {
        const circuit = await tester(
            path.join(__dirname, "circuits", "credential.old-simpletree.circom"),
    ...
    it("Test Credential crowded tree", async () => {
        const circuit = await tester(
            path.join(__dirname, "circuits", "credential.old-crowdedtree.circom"),
            {reduceConstraints: false}
        );
